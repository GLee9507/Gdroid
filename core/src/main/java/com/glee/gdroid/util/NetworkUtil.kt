...
object NetworkUtil {
    ...
    enum class NetType private constructor(var value: Int) {
        None(1),
        Mobile(2),
        Wifi(4),
        Other(8)
    ...
    enum class NetWorkType private constructor(var value: Int) {
        UnKnown(-1),
        Wifi(1),
        Net2G(2),
        Net3G(3),
    ...
    fun isMobileEnabled(context: Context): Boolean {
        if (Build.VERSION.SDK_INT < Build.VERSION_CODES.P) {
            try {
                @SuppressLint("PrivateApi") val getMobileDataEnabledMethod = ConnectivityManager::class.java.getDeclaredMethod("getMobileDataEnabled")
